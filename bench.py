...
CSV, SOURCES, RESULT_MD = RESULTS / "results.csv", ROOT / "SOURCES.md", ROOT / "RESULT.md"
...
def thermal_state():
    if platform.system() == "Darwin":
        try:
            ...
            return f"speedlimit={m.group(1)}" if m else "speedlimit=?"
        except Exception:
            return "speedlimit=?"
    return "?"
...
def build():
    ...
    for src, extra in (("ll7.c", []), ("linpack.c", ["-lm"])):
        out = BUILD / src[:-2]
        cmd = [CC, *FLAGS, "-o", str(out), str(KERNELS / src), *extra]
        print(" ".join(cmd))
...
def run(quick=False):
    ...
    for wid in WORKLOADS:
        ...
        base = dict(workload=wid, precision="fp64", source="measured", compiler=comp,
...
def merge_rows(new, mid):
    """Replace this machine's measured rows; keep every published/reconstructed row."""
    keep = [r for r in read_csv() if not (r["source"] == "measured" and r["machine"].startswith(mid + "-"))]
    write_csv(keep + new)
...
def table():
    ...
    if meas:
        ...
        for comp, flags in dict.fromkeys((r["compiler"], r["flags"]) for r in meas):
            lines.append(f"- `{comp}` · flags `{flags}` · runs per sustained cell: {meas[0]['runs']}")
